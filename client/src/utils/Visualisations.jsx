...
export function BarsViz({ data, peakIndex, accent = "#ffbf69", base = "#2b6b7a" }) {
        ...
        return (
          <g key={d.label}>
            <rect
              ...
              className="viz-grow"
              style={{ transformOrigin: `${x + w / 2}px 60px`, animationDelay: `${i * 60}ms` }}
            />
            <text x={x + w / 2} y={70} fontSize="6" textAnchor="middle" fill="#6f9aa2">
              {d.label}
            </text>
          </g>
        );
    ...
    <div className="space-y-2.5">
      {data.map((d, i) => (
        <div key={d.label} className="flex items-center gap-3">
          <span className="w-24 shrink-0 text-[11px] text-[#a9cdd2]">{d.label}</span>
  ...
  return (
    <div className="flex flex-col items-center">
      <svg viewBox="0 0 100 56" className="w-36">
        <path d="M10 50 A40 40 0 0 1 90 50" fill="none" stroke="#173a45" strokeWidth="8" strokeLinecap="round" />
        <path
          d="M10 50 A40 40 0 0 1 90 50"
          fill="none" stroke="#ff735c" strokeWidth="8" strokeLinecap="round"
          strokeDasharray={`${filled} ${C}`}
        />
        <text x="50" y="46" textAnchor="middle" fontSize="16" fontWeight="800" fill="#fff">
          {value}%
        </text>
      </svg>
